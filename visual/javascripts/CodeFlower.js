...
CodeFlower.prototype.update = function(json) {
  ...
  var g = Math.atan(total / 100) / Math.PI * 0.4;
  this.force
    .gravity(g)
    .nodes(nodes)
    .links(links)
...
CodeFlower.prototype.clicked = function(d) {
  // Toggle children on click.
  console.log(d);
  /*
  if (d.children) {
...
/*
CodeFlower.prototype.showtext = function(d) {
    console.log(this.node);
    var text = svg.selectAll("text")
                     //.data(d3.selectAll("circle"))
                     .data([{x:100, y: 120, name: "hi"},{x: 140, y: 140, name: "ho"}])
                     .enter()
                     .append("text")
                     .attr("x", function(d) { 
                     console.log(d);
...
CodeFlower.prototype.mouseover = function(d) {
  this.text.attr('transform', 'translate(' + d.x + ',' + (d.y - 5 - (d.children ? 3.5 : Math.sqrt(d.size) / 2)) + ')')
    .text(d.name + " (" + d.size + ")") // TODO: polish the label
    .style('display', null);
};
