...
/*
 * To avoid namespace conflicts, 'dragIF_' has been added to those functions without the 'DIF_' prefix
 * - CHARMe project, CGI, 2014
 * 
 */
...
function dragIF_addHandle(o, win) {
	if (arguments.length==2 && win==window) {
		...
		var p = win;
		while (p=p.parent) {
			if (p.dragIF_addHandle) { p.dragIF_addHandle(o,win,true); return; }
			if (p==win.top) { return; } // Already reached the top, stop looking
			}
		return; // If it reaches here, there is no parent with the dragIF_addHandle function defined, so this frame can't be dragged!
		}
	var topRef=win;
...
function dragIF_getSrcElement(e) {
	var tgt = e.target;
...
function dragIF_isHandleClicked(handle, objectClicked) {
...
function DIF_begindrag(e, win) {
	...
	var isHandle = false;
	var t = e.srcElement || dragIF_getSrcElement(e);
	for (var i=0; i<DIF_handles[iframename].length; i++) {
		if (dragIF_isHandleClicked(DIF_handles[iframename][i],t)) {
			isHandle=true;
			break;
			}
		}
